...
class Estadísticas():
    ...
    def experimentos(experimentos: List[Experimento], recetas: List[Recetas]):
        ...
        highest_num=max(result,key=result.get)
        least_num=min(result,key=result.get)
        highest=GestionExperimentos.search_receta(highest_num,recetas)
        least=GestionExperimentos.search_receta(least_num,recetas)
        print(f"""    Experimento más hecho: {highest.name}
    Nro. de veces realizado: {highest_num}
              
    Experimento menos hecho: {least.name}
    Nro. de veces realizado: {least_num}""")
